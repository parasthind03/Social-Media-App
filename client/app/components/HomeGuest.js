		...
		switch (action.type) {
			...
			case 'usernameAfterDelay':
				if (draft.username.value.length < 3) {
					draft.username.hasErrors = true
					draft.username.message = 'Username must have atleast 3 characters.'
				}
				if (!draft.username.hasErrors && !draft.noRequest) {
			...
			case 'emailAfterDelay':
				...
				if (!draft.email.hasErrors && !draft.noRequest) {
			...
			case 'passwordAfterDelay':
				if (draft.password.value.length < 12) {
					draft.password.hasErrors = true
					draft.password.message = 'Password must have atleast 12 characters.'
				}
				return
			case 'submitForm':
				if (
		...
		if (state.submitCount) {
			...
			async function fetchResults() {
				try {
					...
					appDispatch({ type: 'flashMessage', value: 'Congrats! Welcome to the App.' })
				} catch (e) {
					console.log('There was a problem or the request was cancelled.')
				}
			}
